...
const ViewCompany = () => {
  ...
  useEffect(() => {
    ...
    const fetchCompanies = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8081/api/subadmin/all');
  ...
  const confirmDelete = async () => {
    if (companyToDelete) {
      try {
        ...
        const response = await fetch(`http://localhost:8081/api/subadmin/delete/${companyToDelete.id}`, {
          method: 'DELETE'
        });
  ...
  const handleSendLoginDetails = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:8081/api/subadmin/send-email/${company.email}`, {
        method: 'POST'
      });
