...
const CheckImages = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8282/api/employee/${subAdminId}/list`);
  ...
  const fetchEmployeeImage = async () => {
    if (!selectedEmployee || !selectedDate) {
      toast.error(t('checkImages.errors.selectEmployeeAndDate'));
      return;
    }
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8282/api/employee/${subAdminId}/${selectedEmployee}/attendance/images?date=${selectedDate}`
      );
      
      if (response.data.imageUrl) {
        setEmployeeImage(response.data);
        toast.success(t('checkImages.success.imageFound'));
      } else {
        setEmployeeImage(null);
        toast.error(response.data.message || t('checkImages.errors.noImageFound'));
      }
    } catch (error) {
      toast.error(t('checkImages.errors.failedToFetchImage'));
  ...
  const fetchAllImages = async () => {
    if (!selectedDate) {
      toast.error(t('checkImages.errors.selectDate'));
      return;
    }
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8282/api/employee/${subAdminId}/attendance/images/all?date=${selectedDate}`
      );
      
      setAllImages(response.data);
      if (response.data.length === 0) {
        toast.error(t('checkImages.errors.noImagesFound'));
      } else {
        toast.success(`Found ${response.data.length} images`);
      }
    } catch (error) {
      toast.error(t('checkImages.errors.failedToFetchImages'));
  ...
  const handleImageError = (e) => {
    ...
    setImageLoading(false);
    toast.error(t('checkImages.errors.failedToLoadImage'));
  };
        ...
        <div className={`rounded-xl shadow-lg p-6 mb-6 ${
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              }`}>{t('checkImages.title')}</h1>
              <p className={`${
                isDarkMode ? "text-gray-300" : "text-gray-600"
              }`}>{t('checkImages.subtitle')}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-2">
              ...
              }`}>{t('checkImages.selectDate')}:</label>
            </div>
            <input
              type="date"
              value={selectedDate}
        ...
        <div className={`rounded-xl shadow-lg p-6 mb-6 ${
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className={`text-xl font-semibold ${
              isDarkMode ? "text-blue-400" : "text-gray-800"
            }`}>{t('checkImages.searchByEmployee')}</h2>
          </div>
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t('checkImages.searchPlaceholder')}
        ...
        {employeeImage && (
          <div className={`rounded-xl shadow-lg p-6 mb-6 ${
            ...
            <h3 className={`text-lg font-semibold mb-4 ${
              isDarkMode ? "text-blue-400" : "text-gray-800"
            }`}>{t('checkImages.employeeImage')}</h3>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-2/3 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-medium ${
                      isDarkMode ? "text-gray-300" : "text-gray-700"
                    }`}>{t('checkImages.punchIn')}:</span>
                    <span className={isDarkMode ? "text-white" : "text-gray-800"}>{employeeImage.punchInTime || t('checkImages.notRecorded')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className={`w-4 h-4 ${
                      isDarkMode ? "text-blue-400" : "text-blue-600"
                    ...
                    <span className={`font-medium ${
                      isDarkMode ? "text-gray-300" : "text-gray-700"
                    }`}>{t('checkImages.punchOut')}:</span>
                    <span className={isDarkMode ? "text-white" : "text-gray-800"}>{employeeImage.punchOutTime || t('checkImages.notRecorded')}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={`rounded-xl shadow-lg p-6 ${
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className={`text-xl font-semibold ${
                ...
                {t('checkImages.allImagesForDate')} {selectedDate} ({allImages.length})
              </h3>
            </div>
            ...
            <div className={`rounded-xl max-w-7xl max-h-[95vh] overflow-auto ${
              ...
              <div className="p-4">
                  ...
                  {imageLoading && (
                    <div className={`absolute inset-0 flex items-center justify-center ${
                      ...
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className={`text-sm ${
                          isDarkMode ? "text-gray-300" : "text-gray-600"
                        }`}>{t('checkImages.loadingHighQuality')}</p>
                      </div>
                    </div>
                  )}
                ...
                <div className={`mt-4 grid grid-cols-2 gap-4 text-sm ${
                  ...
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.employee')}:</strong> {selectedImageModal.employeeName}</div>
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.date')}:</strong> {selectedImageModal.date}</div>
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.workType')}:</strong> {selectedImageModal.workType}</div>
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.status')}:</strong> {selectedImageModal.status}</div>
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.punchIn')}:</strong> {selectedImageModal.punchInTime || t('checkImages.notRecorded')}</div>
                  <div><strong className={isDarkMode ? "text-white" : "text-gray-900"}>{t('checkImages.punchOut')}:</strong> {selectedImageModal.punchOutTime || t('checkImages.notRecorded')}</div>
                </div>
